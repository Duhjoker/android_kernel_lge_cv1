...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_RECOVERY_ESD)
		TOUCH_I("Execute recovery action. To restore a current status\n");
...
static void lg4894_connect(struct device *dev)
{
	...
	d->charger = 0;
	/* wire */
	if (charger_state == CONNECT_DISCONNECTED)
		d->charger = CONNECT_NONE;
...
//====================================================== //
static void lg4894_int_pin_work_func(struct work_struct *int_pin_work)
{
	struct lg4894_data *d =
			container_of(to_delayed_work(int_pin_work),
				struct lg4894_data, int_pin_work);
	...
	for(i=0 ; i<20; i++) {
		...
		touch_msleep(100);
		if(int_pin_state == 1) {
			TOUCH_I("interrupt pin high triggered. (%dx100)ms\n", i);
			return;
		}
	}
...
#if defined(CONFIG_LGE_DISPLAY_RECOVERY_ESD)
	INIT_DELAYED_WORK(&d->int_pin_work, lg4894_int_pin_work_func);
#endif
...
#if defined(CONFIG_LGE_MODULE_DETECT)
		if (lg4894_panel_type == CV3_LGD) {
			memcpy(fwpath, ts->def_fwpath[1], sizeof(fwpath));
			TOUCH_I("Get Touch Firmware for CV3 LGD(4894A)\n");
		} else if (lg4894_panel_type == CV3_TOVIS) {
			memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath));
			TOUCH_I("Get Touch Firmware for CV3 TOVIS(4894B)\n");
		} else if (lg4894_panel_type == CV1_LGD_LG4894A) {
			memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath));
			TOUCH_I("Get Touch Firmware for CV1 LGD(4894A)\n");
		}
#else
		memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath));
#endif /* CONFIG_LGE_MODULE_DETECT */
...
#if defined(CONFIG_LGE_MODULE_DETECT)
static bool lg4894_get_device_type(void)
{
	bool bdevice = false;
