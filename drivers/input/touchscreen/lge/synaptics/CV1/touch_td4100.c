...
/* touch irq handle according to display power down(shutdown) in mfts */
...
static int synaptics_change_ime_status(struct device *dev)
{
	...
	// Control Noise Mitigation Mode
	synaptics_set_page(dev, ANALOG_PAGE);

	ret = synaptics_write(dev, NOISE_MITIGATION_CTRL_REG, &ime_status, sizeof(ime_status));
	if (ret < 0) {
		TOUCH_E("NOISE_MITIGATION_CTRL_REG write error - ret:%d\n", ret);
		return ret;
	}

	data = 0x04; /* Force Update */
	ret = synaptics_write(dev, ANALOG_COMMAND_REG, &data, sizeof(data));
	if (ret < 0) {
...
static int synaptics_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&d->state.power_seq) == POWER_SEQ_ENTER) {
		TOUCH_I("Skip lpwg mode during POWER_SEQ\n");
		return 0;
	}
...
static int synaptics_delta_esd_condition_set(struct device *dev)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	u8 buffer[6];
	...
	TOUCH_I("%s, max : %d, min : %d, frame_cnt : %d\n", __func__, d->f51_reg.esd_saturated_max, d->f51_reg.esd_saturated_min, d->f51_reg.esd_frame_cnt);

	synaptics_set_page(dev, LPWG_PAGE);

	buffer[0] = (d->f51_reg.esd_saturated_max >> 8) & 0xff;
	buffer[1] = (d->f51_reg.esd_saturated_max >> 0) & 0xff;
	buffer[2] = (d->f51_reg.esd_saturated_min >> 8) & 0xff;
	buffer[3] = (d->f51_reg.esd_saturated_min >> 0) & 0xff;
	buffer[4] = (d->f51_reg.esd_frame_cnt >> 8) & 0xff;
	buffer[5] = (d->f51_reg.esd_frame_cnt >> 0) & 0xff;

	synaptics_write(dev, MAX_POSITIVE_SATURATED_DELTA, buffer, sizeof(buffer));
...
static int synaptics_delta_esd_condition_info(struct device *dev)
{
	struct synaptics_data *d = to_synaptics_data(dev);

	TOUCH_TRACE();

	d->f51_reg.esd_saturated_max = 860;
	d->f51_reg.esd_saturated_min = 64;
	d->f51_reg.esd_frame_cnt = 360;

	synaptics_delta_esd_condition_set(dev);
...
int synaptics_init(struct device *dev)
{
	...
	if (ret < 0) {
		...
		atomic_set(&d->state.power_seq, POWER_SEQ_EXIT);
		return ret;
	}
	...
	synaptics_delta_esd_condition_info(dev);
...
static int synaptics_get_cmd_version(struct device *dev, char *buf)
{
	...
	offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"IC_product_id[%s]\n", d->ic_info.product_id);
	if (synaptics_is_product(d, "PLG665", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : TD4100\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
	...
	if (synaptics_is_img_product(d, "PLG665", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : TD4100\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
...
static int synaptics_notify(struct device *dev, ulong event, void *data)
{
	...
	switch (event) {
		case LCD_EVENT_LCD_BLANK:
			...
			atomic_set(&d->state.power_seq, POWER_SEQ_ENTER);
			break;
...
static int synaptics_shutdown(struct device *dev)
{
	TOUCH_TRACE();
...
static bool td4100_get_device_type(void)
{
	bool bdevice = false;
...
static int __init touch_device_init(void)
{
	...
	TOUCH_TRACE();
#if defined(CONFIG_LGE_MODULE_DETECT)
	if (!td4100_get_device_type()) {
		TOUCH_I("%s, td4100 returned\n", __func__);
		return 0;
	}
	TOUCH_I("This panel is td4100\n");
#endif /* CONFIG_LGE_MODULE_DETECT */
	TOUCH_I("td4100__[%s] touch_bus_device_init\n", __func__);
