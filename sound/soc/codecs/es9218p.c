...
static int  es9218p_standby2lpb(void)
{
    ...
    if( !g_es9218_priv->es9218_data->ext_bypass_mode ) {
#ifdef SHOW_LOGS
    pr_info("%s(): ext_bypass_mode is not present \n", __func__);
#endif
        ...
        struct es9218_data *pdata)
{
    ...
    */
    ...
    /*
     * pre-condition to use bypass-mode to set GPIO2
     * 1. When bypass-mode it's impossible to trigger the elect_irq cause internal resistance of es9218p.
     * 2. The external switch is installed to the schematic of the japan carrier to avoid the interfarerence of es9218p.
    */
    if( of_property_read_bool(dev->of_node, "dac,use-ext-bypass-mode") ) {
        pdata->ext_bypass_mode = true;
    } else {
        pdata->ext_bypass_mode = false;
    }

#ifdef SHOW_LOGS
    pr_info("%s: ext_bypass_mode is %s\n", __func__, pdata->ext_bypass_mode ? "true":"false");
#endif
